function [serie_an, v_min, pro_mm, rango_ev, f_min, fin_ev, ini_ev, f_min_jul, minimos, min_serie] = dryev(serie, v1, v2)
    // rango recomendado, para el otoñillo, v1 = 182 (1-Jul), v2 = 304 (31-Oct)
    a = 1
    back = []
    forw = []
    [nf, nc] = size(serie);
        ...
        for e = 30:30 // rango total 4:nc-1
            ...
            if serie_an(v1,4)<=min_serie(1,4) | serie_an(v2,4)<=min_serie(1,4) | mean(serie_an(v1:v2,4))==0 | size(min_serie,'r')>1 then
                ...
                elseif size(min_serie,'r')>1 then
                    mprintf("ALERTA: Se encontró  que en la estación %i el año %i tiene %i valores mínimos entre\n los días %i y %i, se va a sustituir los valores de las saludas para este año como Nan.\n",e-3,serie_an(v2,1), size(min_serie,'r'),v1,v2)
                end
